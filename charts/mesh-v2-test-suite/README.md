...
- `mesh-v2-test-suite-java-otel-demo`：承载 Java OTel 示例服务的 manifest（Instrumentation + consumer / provider / asm-client），需要时由用户用 `kubectl apply` 一键部署，详见 [部署 Java OTel 示例服务](#部署-java-otel-示例服务)。
...
└── templates/
    ...
    └── java-otel-demo-configmap.yaml    承载 files/java-otel-demo/ 两个 manifest 的 ConfigMap
```
...
部署完成后即可在 Jaeger UI 上看到 consumer / provider 之间的调用链。

### 修改示例服务清单

- 源文件位于 [`files/java-otel-demo/`](./files/java-otel-demo/)，是带 Helm 模板语法的 manifest。
- ConfigMap 模板 [`templates/java-otel-demo-configmap.yaml`](./templates/java-otel-demo-configmap.yaml) 通过 `tpl + .Files.Get` 把上述源文件渲染后嵌入 `data` 字段；所有镜像引用通过 [`_helpers.tpl`](./templates/_helpers.tpl) 的 `mesh-v2-test-suite.image` helper 从 `values.yaml` 拉取，因此换 registry / 升级镜像版本只需改 `values.yaml`，无需动 manifest 本体。
