...
1.  Create a namespace called `egress-gateway`:
    ...
    kubectl label namespace egress-gateway pod-security.kubernetes.io/enforce=restricted
    ```
...
6.  Add the `istio-waypoint` gateway class overlay to the `Istio` resource so that the waypoint pod is admitted into the `Restricted` `egress-gateway` namespace:
