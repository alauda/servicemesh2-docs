...
test_routing_egress_traffic_via_k8s_gateway_api_in_sidecar_mode() {
    ...
    # 步骤 2: 为 istio 网关类打 seccompProfile overlay
    # egress-gateway 命名空间启用 Restricted PSA，网关 pod 需要 RuntimeDefault seccomp 才能准入；
    # 必须在创建 Gateway 之前打，否则网关 Deployment 会被准入控制拒绝
    log_info "步骤 2: 为 istio 网关类打 seccompProfile overlay"
    runme run sidecar-egress:patch-gatewayclass || {
        log_error "配置 istio 网关类 seccompProfile 失败"
        return 1
    }

    # 步骤 3: 写入 egress 网关 CR YAML（ServiceEntry + Gateway + 2×HTTPRoute；无占位符）+ 应用
    log_info "步骤 3: 部署 egress 网关 CR (ServiceEntry/Gateway/HTTPRoute)"
    ...
    # 步骤 4: (仅 ENABLE_GW_LINUX_KERNEL_COMPAT=true 生效) egress 网关监听 80 特权端口，
    ...
    # 步骤 5: 等待 K8s Gateway API 自动置备的 egress 网关 Deployment 就绪
    log_info "步骤 5: 等待 egress 网关 Deployment 就绪"
    _wait_for_deployment egress-gateway httpbin-egress-gateway-istio

    # 步骤 6: 验证 Gateway 状态（PROGRAMMED=True）
    # 输出含动态值（ADDRESS、AGE），使用 __cmp_lines 校验关键字段
    log_info "步骤 6: 验证 Gateway 状态"
    local gw_output
...
EOF
    ...
    # 步骤 8: 为 curl 命名空间启用 sidecar 注入（InPlace 升级策略）
    log_info "步骤 8: 启用 sidecar 注入"
    ...
    # 步骤 10: 获取 curl pod 名称（文档块已 export，eval 后即为导出变量）
