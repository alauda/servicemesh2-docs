...
2. **Run the gateway as root**
   - Set `runAsUser: 0`, `runAsGroup: 0`, and `runAsNonRoot: false` in the container's `securityContext`. This grants full privileges to the gateway process. Evaluate the security implications carefully before using this approach.
   - Also set `runAsNonRoot: false` on the `istio-proxy` container in your `gateway-deployment.yaml` (or remove the field) to keep it consistent with this template. Otherwise its container-level `runAsNonRoot: true` overrides the template, and the gateway pod is rejected with `container's runAsUser breaks non-root policy`.
   - Running the gateway as root is incompatible with the `Restricted` Pod Security Admission profile, which forbids root containers. The gateway namespace must enforce the `Baseline` profile or lower. For more information, see [Namespace Requirements](../../installing/installing-service-mesh/application-deployment/namespace-requirements.mdx#pod-security-admission-modes).
:::
