...
1. Create a YAML file named `gateway-injection-template.txt` that contains the default injection template for gateways.
   ...
   <details>
     ...
     ```yaml title="gateway-injection-template.txt" {name=kernel-compat:istio-injection-template}
     {{- $containers := list }}
...
:::note
If you run the gateway as root, the gateway `Deployment` you create when [installing a gateway via gateway injection](./installing-a-gateway-via-injection.mdx) must use a `securityContext` that stays **consistent** with this compatibility configuration. A container-level `runAsNonRoot: true` on the `istio-proxy` container in `gateway-deployment.yaml` takes precedence over this patched injection template: if it remains `true` while the template runs the proxy as root (`runAsUser: 0`), the gateway pod is rejected and the rollout never completes, with an error similar to `container's runAsUser breaks non-root policy (container: istio-proxy)`.

To avoid this, set `runAsNonRoot: false` on the `istio-proxy` container in `gateway-deployment.yaml` (or remove the field) so that it matches the root configuration applied here. This consistency requirement applies only to the Istio gateway injection method; the Kubernetes Gateway API method described below is configured entirely through the `asm-kube-gateway-options` ConfigMap and is not affected.
:::
...
2. Patch the default `gateway` injection template for `Istio` resource:
   ...
   ```bash {name=kernel-compat:patch-istio-template}
...
3. Wait for the control plane to return the `Ready` status condition by running the following command:
   ...
   ```bash {name=kernel-compat:wait-istio-ready}
...
1. Create a ConfigMap named `asm-kube-gateway-options` in the same namespace where you plan to deploy your Gateway:
   ...
   ```yaml {name=kernel-compat:k8s-gateway-options-scenario1}
   apiVersion: v1
   kind: ConfigMap
   metadata:
...
```yaml {name=kernel-compat:k8s-gateway-options-scenario2}
apiVersion: v1
kind: ConfigMap
metadata:
  name: asm-kube-gateway-options
