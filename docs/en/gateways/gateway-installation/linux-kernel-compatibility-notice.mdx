...
2. **Run the gateway as root**
   ...
   - Running the gateway as root is incompatible with the `Restricted` Pod Security Admission profile, which forbids root containers. The gateway namespace must enforce the `Baseline` profile or lower. For more information, see [Pod Security Admission](../../installing/pod-security-admission.mdx).
:::
