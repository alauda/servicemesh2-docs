...
- An `IstioCNI` resource is created.
- Confirm [Linux kernel compatibility](./linux-kernel-compatibility-notice.mdx#linux-kernel-compatibility-notice).
...
1. Create a namespace for the gateway:
   ...
   ```bash {name=install-gateway-injection:create-namespace}
...
2. Create a YAML file named `secret-reader.yaml` that defines the service account, role, and role binding for the gateway deployment. These settings enable the gateway to read the secrets, which is required for obtaining TLS credentials.
   ...
   ```yaml title="secret-reader.yaml" {name=install-gateway-injection:secret-reader-yaml}
   apiVersion: v1
   kind: ServiceAccount
   metadata:
     name: secret-reader
...
4. Apply the YAML file by running the following command:
   ```bash {name=install-gateway-injection:apply-secret-reader}
...
5. Create a YAML file named `gateway-deployment.yaml` that defines the Kubernetes `Deployment` object for the gateway.
   ...
   ```yaml title="gateway-deployment.yaml" {name=install-gateway-injection:gateway-deployment-yaml}
   apiVersion: apps/v1
   kind: Deployment
   metadata:
     name: <gateway_name>
   ...
   <Callouts>
     ...
     5. Adds the `RuntimeDefault` seccomp profile so that the gateway pod satisfies the `Restricted` Pod Security Admission profile.
        The `gateway` injection template does not read `spec.values.global.proxy.seccompProfile` from the `Istio` resource, so the profile must be set on the gateway deployment itself.
        Remove this field if the gateway namespace does not enforce the `Restricted` profile.
     6. Sets the `serviceAccountName` to the name of the `ServiceAccount` created previously.
     7. (Optional) Uncomment to set node selectors that schedule the gateway pods on <ExternalSiteLink name="acp" href="/configure/clusters/acp-node-planning.html" children="Infra Nodes" />.
     8. (Optional) Uncomment to set tolerations that allow the gateway pods to be scheduled on <ExternalSiteLink name="acp" href="/configure/clusters/acp-node-planning.html" children="Infra Nodes" />.
   </Callouts>

6. Apply the YAML file by running the following command:

   ```bash {name=install-gateway-injection:apply-gateway-deployment}
   kubectl apply -f gateway-deployment.yaml
   ```

7. Verify that the gateway `Deployment` rollout was successful by running the following command:

   ```bash {name=install-gateway-injection:verify-rollout}
...
8. Create a YAML file named `gateway-service.yaml` that contains the Kubernetes `Service` object for the gateway.
   ...
   ```yaml title="gateway-service.yaml" {name=install-gateway-injection:gateway-service-yaml}
   apiVersion: v1
   kind: Service
   metadata:
     name: <gateway_name>
...
10. *Optional* : Create a YAML file named `gateway-hpa.yaml` that defines a horizontal pod autoscaler for the gateway.
    ...
    ```yaml title="gateway-hpa.yaml" {name=install-gateway-injection:gateway-hpa-yaml}
...
11. *Optional* : Apply the YAML file by running the following command:
    ...
    ```bash {name=install-gateway-injection:apply-gateway-hpa}
...
12. *Optional* : Create a YAML file named `gateway-pdb.yaml` that defines a pod disruption budget for the gateway.
    ...
    ```yaml title="gateway-pdb.yaml" {name=install-gateway-injection:gateway-pdb-yaml}
...
13. *Optional* : Apply the YAML file by running the following command:
    ...
    ```bash {name=install-gateway-injection:apply-gateway-pdb}
