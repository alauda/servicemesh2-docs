...
:::info
On some operating systems (e.g., CentOS 7) with older Linux kernels, gateways may not be able to listen on ports below 1024. It is recommended to configure your gateway to use ports above 1024 to avoid privilege requirements. If you must use privileged ports (below 1024), you can configure the gateway to run as root user by creating a ConfigMap with custom security context settings as described below.
:::
