...
1.  Create a namespace called `httpbin`:
    ...
    kubectl label namespace httpbin pod-security.kubernetes.io/enforce=restricted
    ```
...
5.  Add the `istio` and `istio-waypoint` gateway class overlays to the `Istio` resource so that the gateway and waypoint pods are admitted into the `Restricted` `httpbin` namespace:
...
1.  Create a namespace for a `curl` client:
    ...
    kubectl label namespace curl pod-security.kubernetes.io/enforce=restricted
    ```
