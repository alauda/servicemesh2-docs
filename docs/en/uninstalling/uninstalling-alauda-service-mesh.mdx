...
1.  Get the `Istio` resource name by executing this command:
    ...
    ```shell {name=uninstall-mesh:get-istio}
    kubectl get istio
    ```

    **Example output**

    ```shell {name=uninstall-mesh:get-istio-output}
    NAME                         NAMESPACE      PROFILE   REVISIONS   READY   IN USE   ACTIVE REVISION   STATUS    VERSION   AGE
    <name_of_custom_resource>   istio-system             1           1       1        default           Healthy   v1.30.3   1d
    ```
...
2.  Use the name from the previous command's output to replace `<name_of_custom_resource>`, and then run the following to remove it:
    ...
    ```shell {name=uninstall-mesh:delete-istio}
    kubectl delete istio/<name_of_custom_resource>
    ```

    **Example output**

    ```shell {name=uninstall-mesh:delete-istio-output}
...
1.  Get the `IstioCNI` resource by executing this command:
    ...
    ```shell {name=uninstall-mesh:get-istiocni}
    kubectl get istioCNI
    ```

    **Example output**

    ```shell {name=uninstall-mesh:get-istiocni-output}
    NAME      NAMESPACE   PROFILE   READY   STATUS    VERSION   AGE
    default   istio-cni             True    Healthy   v1.30.3   1d
    ```

2.  Run the following to remove it:

    ```shell {name=uninstall-mesh:delete-istiocni}
    kubectl delete istioCNI/default
    ```

    **Example output**

    ```shell {name=uninstall-mesh:delete-istiocni-output}
...
1.  Delete the `istio-system` namespace with the following command:
    ...
    ```shell {name=uninstall-mesh:delete-ns-istio-system}
    kubectl delete ns istio-system
    ```

    **Example output**

    ```shell {name=uninstall-mesh:delete-ns-istio-system-output}
    namespace "istio-system" deleted
    ```

2.  Delete the `istio-cni` namespace with the following command:

    ```shell {name=uninstall-mesh:delete-ns-istio-cni}
    kubectl delete ns istio-cni
    ```

    **Example output**

    ```shell {name=uninstall-mesh:delete-ns-istio-cni-output}
...
1.  Delete the `servicemesh-operator2` subscription:
    ...
    ```shell {name=uninstall-mesh:delete-subscription}
    kubectl delete subscription servicemesh-operator2 -n sail-operator
    ```

    **Example output**

    ```shell {name=uninstall-mesh:delete-subscription-output}
...
1.  To delete the Istio CRDs, execute the command below:
    ...
    ```shell {name=uninstall-mesh:delete-crds}
