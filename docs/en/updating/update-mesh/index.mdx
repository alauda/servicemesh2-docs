...
2. Install bundle version `v1.5.1` from the channel that the previous command reported. Keep the channel unchanged: the channel selects which resources and which API versions the cluster serves, and both channels serve `TLSRoute` at `v1`.
   ...
   Use `--server-side`. A client-side apply fails on the `HTTPRoute` CRD with `metadata.annotations: Too long`, because the definition exceeds the 256 KB limit of the `kubectl.kubernetes.io/last-applied-configuration` annotation, which leaves the bundle partially upgraded. Add `--force-conflicts` if the CRDs were previously installed with a client-side apply.
...
:::note
The `v1.5.x` bundle installs a `safe-upgrades.gateway.networking.k8s.io` validating admission policy that rejects any later attempt to install a bundle earlier than `v1.5.0`. Rolling the CRDs back takes two steps: delete the policy and its binding, and remove `v1` from `status.storedVersions` on the `TLSRoute` CRD, which the earlier bundle does not define. Delete your `TLSRoute` resources before you patch that field and recreate them afterward, because it records the version they are persisted at.
:::
