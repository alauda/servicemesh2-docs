...
Istio 1.30 depends on Kubernetes Gateway API `v1.5.1` and reads `TLSRoute` from the standard channel (`gateway.networking.k8s.io/v1`). Istio 1.28 read it from `v1alpha2`.

Upgrade the Gateway API CRDs to bundle version `v1.5.x` **before** you update the control plane to Istio 1.30. Otherwise `TLSRoute` resources become invisible to the control plane: TLS passthrough `Gateway` listeners silently report `status.listeners[].attachedRoutes: 0` and the corresponding Envoy listener is never programmed. `HTTPRoute`, `GRPCRoute`, and `ReferenceGrant` are not affected.

**Procedure**

1. Check the bundle version installed in the cluster:
...
2. If the version is earlier than `v1.5.0`, install the CRDs from the standard channel:

   ```bash
   kubectl apply -k "github.com/kubernetes-sigs/gateway-api/config/crd?ref=v1.5.1"
   ```

   If the cluster uses the experimental channel, apply `config/crd/experimental` instead.
