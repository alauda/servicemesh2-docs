...
1. Create the `istio-cni` and `istio-system` namespace by running the following command:
   ...
   ```bash {name=update-revisionbased:create-namespaces}
...
2. Install the Istio CNI plugin with the desired version. The following example configuration creates an `IstioCNI` resource named default in the `istio-cni` namespace:
   ...
   ```yaml {name=update-revisionbased:create-istio-cni}
...
3. Deploy the Istio control plane using the `RevisionBased` update strategy. The following example configuration creates an `Istio` resource named `default` in the `istio-system` namespace:
   ...
   **Example configuration**
   ...
   ```yaml {name=update-revisionbased:create-istio}
   cat <<EOF | kubectl apply -f -
   apiVersion: sailoperator.io/v1
   kind: Istio
   metadata:
...
4. Get the `IstioRevision` name by running the following command:
   ...
   ```bash {name=update-revisionbased:get-istiorevision}
   kubectl get istiorevision -n istio-system
   ```

   **Example output**

   ```text {name=update-revisionbased:get-istiorevision-output}
...
5. Set up the application workloads to execute in the cluster. For instance, you can deploy the `bookinfo` sample application into the `bookinfo` namespace.
   ...
   ```shell {name=update-revisionbased:create-bookinfo-ns}
   ...
   ```shell {name=update-revisionbased:label-bookinfo-ns}
   ...
   ```shell {name=update-revisionbased:deploy-bookinfo}
...
6. Inspect the `Istio` resource using the following command:
   ...
   ```shell {name=update-revisionbased:get-istio-install}
   kubectl get istio -n istio-system
   ```

   **Example output**

   ```text {name=update-revisionbased:get-istio-install-output}
...
7. Confirm that the proxy version matches the control plane version by running the following command:
   ...
   ```bash {name=update-revisionbased:verify-proxy-install}
   # <revision_name> example: default-v1-28-6
   ...
   **Example output**
   ...
   ```text {name=update-revisionbased:verify-proxy-install-output}
...
1. Change the version in the `Istio` resource. For example, to update to Istio `1.30.4`, set the spec.version field to `v1.30.4` by running the following command:
   ...
   ```bash {name=update-revisionbased:patch-istio-version}
...
2. Confirm that both `Istio` and `IstioRevision` resources are ready with the new revision.
   ...
   a. Confirm that `Istio` resource is ready by running the following command:
      ...
      ```bash {name=update-revisionbased:get-istio-update}
      kubectl get istio
      ```

      **Example output**

      ```text {name=update-revisionbased:get-istio-update-output}
      NAME      NAMESPACE      PROFILE   REVISIONS   READY   IN USE   ACTIVE REVISION   STATUS    VERSION   AGE
   ...
   b. Confirm that `IstioRevision` resource is ready by running the following command:
      ...
      ```bash {name=update-revisionbased:get-istiorevision-update}
      kubectl get istiorevision
      ```

      **Example output**

      ```text {name=update-revisionbased:get-istiorevision-update-output}
      NAME              NAMESPACE      PROFILE   READY   STATUS    IN USE   VERSION   AGE
...
3. Confirm that there are two control plane pods running, one for each revision by running the following command:
   ...
   ```bash {name=update-revisionbased:get-pods-update}
   kubectl get pods -n istio-system
   ```

   **Example output**

   ```text {name=update-revisionbased:get-pods-update-output}
...
4. Confirm that the workload sidecars are still connected to the previous control plane by running the following command:
   ...
   ```bash {name=update-revisionbased:verify-proxy-old}
   # <revision_name> example: default-v1-28-6
   istioctl proxy-status --revision <revision_name>
   ```
   ...
   **Example output**
   ...
   ```text {name=update-revisionbased:verify-proxy-old-output}
   NAME                                        CLUSTER        CDS                LDS                EDS                RDS                ECDS        ISTIOD                                      VERSION
...
6. Move the workloads to the new control plane by updating the `istio.io/rev` label on the application namespace or pods to the revision name. For example, update the label for the entire namespace by running the following command:

   ```bash {name=update-revisionbased:migrate-workloads}
   # <new_revision_name> example: default-v1-30-4
   kubectl label namespace bookinfo istio.io/rev=<new_revision_name> --overwrite
   ```

7. Restart the application workloads so that the new version of the sidecar gets injected by running the following command:

   ```bash {name=update-revisionbased:restart-workloads}
...
1. Verify that the new version of the sidecar is running by entering the following command:
   ...
   ```bash {name=update-revisionbased:verify-proxy-new}
   # <new_revision_name> example: default-v1-30-4
...
After completing your verification and experimentation, you should remove the RevisionBased update configuration to clean up the development environment and release resources.

**Procedure**

Execute the following commands to remove all Istio components and the sample applications:

```bash {name=update-revisionbased:cleanup}
