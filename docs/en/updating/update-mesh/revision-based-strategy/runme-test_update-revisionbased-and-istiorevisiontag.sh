...
EOF
    ...
    runme run update-revisionbased-tag:create-bookinfo-ns || {
...
EOF
    ...
    output=$(runme run update-revisionbased-tag:verify-proxy-install 2>&1)
    if ! __cmp_lines "$output" "$(cat <<EOF
+ 1.26.3
+ details-v1
+ productpage-v1
...
EOF
    ...
    output=$(runme run update-revisionbased-tag:verify-proxy-old 2>&1)
    if ! __cmp_lines "$output" "$(cat <<EOF
+ 1.26.3
+ details-v1
+ productpage-v1
...
EOF
    ...
    output=$(runme run update-revisionbased-tag:verify-proxy-new 2>&1)
    if ! __cmp_lines "$output" "$(cat <<EOF
+ 1.28.6
+ details-v1
+ productpage-v1
