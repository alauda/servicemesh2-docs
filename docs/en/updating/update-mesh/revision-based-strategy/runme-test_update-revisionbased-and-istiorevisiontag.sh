...
EOF
    ...
    # 13. 更新 Istio 版本至 v1.30.4
    log_info "步骤 13: 更新 Istio 版本至 v1.30.4"
    ...
    for i in $(seq 1 12); do
        NEW_REV=$(runme run update-revisionbased-tag:get-istiorevision-update 2>&1 \
            | grep 'v1.30.4' | grep -oE 'default-v[0-9]+-[0-9]+-[0-9]+' | head -1 || true)
        [ -n "$NEW_REV" ] && break
        sleep 5
    done
    if [ -z "$NEW_REV" ]; then
...
EOF
    ...
    log_info "步骤 20: 更新 Istio CNI 插件至 v1.30.4"
...
EOF
    ...
    if ! retry_runme_verify update-revisionbased-tag:verify-pods __cmp_lines "$(cat <<EOF
+ istiod-$NEW_REV
- istiod-$REV
EOF
    )" 12 5; then
