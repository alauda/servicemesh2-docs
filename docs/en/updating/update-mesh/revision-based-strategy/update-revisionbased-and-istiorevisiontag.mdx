...
- If you use the Kubernetes Gateway API `TLSRoute` or `ListenerSet` resources, you have reviewed [Gateway API CRD requirements](../index.mdx#gateway-api-crd-requirements). Meshes that use only `Gateway` and `HTTPRoute` need no change.
...
2. Confirm that both `Istio` and `IstioRevision` resources are ready with the new revision.
   ...
   a. Confirm that `Istio` resource is ready by running the following command:
      ...
      **Example output**
      ...
      default   istio-system             2           2       2        default-v1-30-3   Healthy   v1.30.3   12m
      ```
   ...
   b. Confirm that `IstioRevision` resource is ready by running the following command:
      ...
      **Example output**
      ...
      default-v1-30-3   istio-system             True    Healthy   True     v1.30.3   52s
      ```
   ...
   c. Confirm that `IstioRevisionTag` resource is ready by running the following command:
      ...
      **Example output**
      ...
      NAME      STATUS    IN USE   REVISION          AGE
      default   Healthy   True     default-v1-30-3   9m53s
      ```
...
3. Confirm that there are two control plane pods running, one for each revision by running the following command:
   ...
   **Example output**
   ...
   istiod-default-v1-30-3-79b66bfb5f-2nq6q   1/1     Running   0          108s
   ```
...
1. Verify that the new version of the sidecar is running by entering the following command:
   ...
   # <new_revision_name> example: default-v1-30-3
