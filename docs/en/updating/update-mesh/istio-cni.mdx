...
- `spec.version`
  ...
  defines the CNI plugin version to install. Specify the value in the format `vX.Y.Z`, where `X.Y.Z` represents the desired version. For example, use `v1.30.4` to install the CNI plugin version `1.30.4`.
...
1. Change the version in the `Istio` resource. For example, to update to Istio `1.30.4`, set the spec.version field to `v1.30.4` by running the following command:
   ...
   ```bash {name=update-istio-cni:patch-version}
   kubectl patch istiocni default --type='merge' -p '{"spec":{"version":"v1.30.4"}}'
   ```

2. Watch the `istio-cni-node` DaemonSet roll out the new version by running the following command:

   ```bash {name=update-istio-cni:rollout-status}
   kubectl rollout status daemonset/istio-cni-node -n istio-cni
   ```

3. After the rollout completes, wait until the `IstioCNI` resource reports the `Ready` condition by running the following command:

   ```bash {name=update-istio-cni:wait-ready}
   kubectl wait --for=condition=Ready istiocnis/default --timeout=5m
   ```

4. Confirm that the new version of the CNI plugin is ready by running the following command:

   ```bash {name=update-istio-cni:get-istiocni}
   kubectl get istiocni default
   ```

   **Example Output**

   ```text {name=update-istio-cni:get-istiocni-output}
   NAME      NAMESPACE   PROFILE   READY   STATUS    VERSION   AGE
   default   istio-cni             True    Healthy   v1.30.4   30m
   ```

5. Inspect the resulting CNI pods by running the following command:

   ```bash {name=update-istio-cni:get-pods}
   kubectl get pods -n istio-cni
   ```

   **Example Output**

   ```text {name=update-istio-cni:get-pods-output}
