...
- If you use the Kubernetes Gateway API `TLSRoute` or `ListenerSet` resources, you have reviewed [Gateway API CRD requirements](../update-mesh/index.mdx#gateway-api-crd-requirements). Meshes that use only `Gateway` and `HTTPRoute` need no change.
