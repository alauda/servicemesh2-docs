...
- kube-ovn is v4.1.5 or later and uses overlay networking. Ambient mode does not work on a kube-ovn underlay network; see [Ambient mode does not work on a kube-ovn underlay network](../../about/release-notes/alauda-service-mesh-v2.2-known-issues.mdx#ambient-mode-requires-kube-ovn-overlay).
...
## Updating the Kiali tracing configuration \{#updating-the-kiali-tracing-configuration}
...
If Alauda Build of Kiali is installed with tracing integration (see [Integrating distributed tracing platform with Alauda Build of Kiali](../../integration/observability/kiali.mdx#integrating-distributed-tracing-platform-with-alauda-build-of-kiali)), enable the setting:

1. Patch the `Kiali` resource:

    ```bash {name=update-ambient:enable-kiali-waypoint-tracing}
    ...
    **Example output**
    ...
    ```text {name=update-ambient:enable-kiali-waypoint-tracing-output}
...
2. Wait for the Kiali Operator to reconcile the configuration and roll out the Kiali deployment:
    ...
    ```bash {name=update-ambient:wait-kiali-waypoint-tracing}
