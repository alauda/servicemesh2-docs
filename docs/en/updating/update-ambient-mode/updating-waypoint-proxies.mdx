...
```text
waypoint-5d9c8b7f9-abc12.bookinfo     Kubernetes     SYNCED (31s)     SYNCED (31s)     SYNCED (28s)     SYNCED (31s)     IGNORED     istiod-57559f96c4-v2h9g     1.28.6-asm-r0
```

The output lists the waypoint pod together with its synchronization state for each configuration type, the istiod pod it is connected to, and the proxy version it runs. `SYNCED` in every column confirms that the waypoint receives configuration from the control plane, and the `VERSION` column should now show the updated release.
...
## Verifying cross-namespace waypoint configuration

If workloads use a waypoint that lives in a different namespace, make sure the waypoint labels are present and correct before you update, so that traffic keeps flowing through the waypoint afterwards. The following example checks the `bookinfo` namespace, which uses the waypoint `waypoint-common` from the `common-infrastructure` namespace; see [Waypoint proxies](../../installing/ambient-mode/waypoint-proxies.mdx) for the setup.

**Procedure**

1. Inspect the waypoint-related labels on the namespace:

    ```bash
    kubectl get ns bookinfo --show-labels | grep waypoint
    ```

2. If either label is missing or points to the wrong waypoint, re-apply them:

    - Set the namespace where the waypoint resides:

        ```bash
        kubectl label ns bookinfo istio.io/use-waypoint-namespace=common-infrastructure --overwrite
        ```

    - Set the waypoint to use:

        ```bash
        kubectl label ns bookinfo istio.io/use-waypoint=waypoint-common --overwrite
        ```
