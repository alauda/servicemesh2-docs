...
waypoint-7b6bcffd55-lj54j.bookinfo     Kubernetes     istiod-85c76b5b66-qgbrv     1.30.3-asm-r3     4 (CDS,LDS,EDS,WDS)
```
...
:::note
Starting with Istio 1.30, waypoint proxies report trace spans under the waypoint's own service name instead of the destination service name. If you use tracing with Kiali, complete the steps in [Updating the Kiali tracing configuration](./updating-ambient-components.mdx#updating-the-kiali-tracing-configuration) after the update, otherwise the trace views of ambient workloads stay empty.
:::
