...
The table below contains the complete upgrade path for a mesh in ambient mode. Ambient mode was introduced in Alauda Service Mesh v2.1, so the path starts at that release. When upgrading, you need to upgrade the Operator versions in sequence, and update the mesh components to the Istio version of each step before you move on to the next one.
...
:::warning
Istio 1.30 requires Kubernetes 1.32 or later. Before you update the mesh components to Istio 1.30, verify that the cluster meets this requirement and review the [compatibility and support matrix](../../about/release-notes/compatibility-support-matrix.mdx).
:::
