...
# 依赖：上游 updating-ambient-components --no-cleanup 已完成 v1.30.4 升级（含 ambient bookinfo），
...
EOF
    ...
    log_success "waypoint 代理版本验证通过 (1.30.4)"
...
EOF
    ...
    if ! retry_runme_verify update-waypoint:verify-get-denied __cmp_contains "$rbac_expected" 24 5; then
