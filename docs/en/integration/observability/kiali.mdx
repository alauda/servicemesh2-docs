...
<Steps>
  ...
  **Example output:**
  ...
  spec:
    ...
    kiali_feature_flags:
      authz:
        require_namespace_get: true  # [!code callout]
      ui_defaults:
        i18n: # [!code callout]
          language: en
          show_selector: true
  ...
  <Callouts>
    ...
    14. When `require_namespace_get` is `true`, the namespace dropdown only shows namespaces for which the user has `get` permission, providing stricter access control in multi-tenant environments. When `false` (the upstream default), users with `list` permission on namespaces are treated as also having `get` permission, which can expose namespaces that users should not access when the two permissions differ.
    15. `i18n` specifies the default language and whether to show the language selector.
  </Callouts>
