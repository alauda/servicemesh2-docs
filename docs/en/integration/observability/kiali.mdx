...
A single Kiali instance can observe a mesh that spans several clusters. See [Installing Kiali in a multi-cluster mesh](../../installing/multi-cluster/install-kiali-multi-cluster/install-kiali-in-multi-cluster-mesh.mdx).
...
CSV="$(kubectl -n kiali-operator get subscription kiali-operator -o jsonpath='{.status.currentCSV}')"
kubectl -n kiali-operator wait --for=create "csv/${CSV}" --timeout=2m
kubectl -n kiali-operator wait --for=jsonpath='{.status.phase}'=Succeeded "csv/${CSV}" --timeout=3m
```

:::note
Wait for the resource to be created before waiting for its phase. `kubectl wait --all` fails with `error: no matching resources found` when it runs before the Operator Lifecycle Manager has created the `ClusterServiceVersion`, which is what happens right after an install plan is approved.
:::
...
## Configuring Monitoring with Kiali \{#configuring-monitoring-with-kiali}

The following steps show how to integrate the Alauda Build of Kiali with monitoring.
...
<Steps>
  ...
  ```bash {name=config-kiali:get-ca-certificate}
  ...
  <Directive type="danger" title="CA certificate missing">
  If the command returns an empty output, try using the following command or contact your administrator to obtain the ACP CA certificate.

  ```bash {name=config-kiali:get-ca-certificate-alternative}
  ...
  **Example output:**
  ...
  ```bash {name=config-kiali:label-istio-system-project-label}
  ...
  spec:
    server:
      ...
      # Set if OIDC login fails because Kiali derives the redirect_uri incorrectly [!code callout]
      # web_fqdn: <platform-fqdn>  [!code callout]
      # web_schema: https
    auth:
      openid:
  ...
  <Callouts>
    ...
    3. `web_fqdn` is the public domain name for accessing the Kiali dashboard (the domain part of the platform URL). Kiali derives it from request headers by default; set it only if the derived OIDC `redirect_uri` is incorrect and login fails.
    4. `web_schema` is the public URL scheme for accessing the Kiali dashboard, also derived from request headers by default; set it together with `web_fqdn`.
    5. `api_proxy` points to **erebus** to map ACP user tokens to Kubernetes tokens.
    6. `api_proxy_ca_data` is the base64‑encoded CA certificate used by **erebus**.
    7. `issuer_uri` is the OIDC issuer URL for **dex**.
    8. `client_id` is the OIDC client ID for **dex**.
    9. `replicas` specifies the number of replicas for the Kiali deployment, should be at least `2` in production environments. In that case, also uncomment `pod_disruption_budget` (available since Alauda Build of Kiali v2.27) so that voluntary node drains keep at least one Kiali pod running.
    10. `class_name` is the ingress class name for the Kiali ingress.
    11. Required in multi cluster mesh. `<mesh_id>` should be the same as the `.spec.values.global.meshId` in `Istio` resource. See [Installing Kiali in a multi-cluster mesh](../../installing/multi-cluster/install-kiali-multi-cluster/install-kiali-in-multi-cluster-mesh.mdx).
    12. `username` references the monitoring basic‑auth username stored in the `kiali-monitoring-basic-auth` Secret.
    13. `password` references the monitoring basic‑auth password stored in the `kiali-monitoring-basic-auth` Secret.
    14. Define `thanos_proxy` if Prometheus is to be queried through a Thanos proxy (it is required when using VictoriaMetrics).
    15. `url` is the monitoring endpoint for Prometheus or VictoriaMetrics.
    16. `i18n` specifies the default language and whether to show the language selector.
  </Callouts>
...
## Integrating distributed tracing platform with Alauda Build of Kiali \{#integrating-distributed-tracing-platform-with-alauda-build-of-kiali}
...
1. Update the `Kiali` resource spec configuration for tracing:
   ...
   **Example `Kiali` resource `spec` configuration for tracing**
   ...
   ```yaml {name=config-kiali:tracing-yaml} title="kiali_cr.yaml"
   spec:
     external_services:
       tracing:
         # query_scope only required in multi cluster
         ...
         use_grpc: true
         use_waypoint_name: true  # [!code callout]
         ...
         # (Optional) Public facing URL of Jaeger [!code callout]
         # external_url: "<platform-url>/clusters/<cluster-name>/jaeger"
   ```

   <Callouts>
     1. Required in multi cluster mesh. `<mesh_id>` should be the same as the `.spec.values.global.meshId` in `Istio` resource. See [Installing Kiali in a multi-cluster mesh](../../installing/multi-cluster/install-kiali-multi-cluster/install-kiali-in-multi-cluster-mesh.mdx).
     2. Specifies whether tracing is enabled.
     3. Specifies the tracing provider (`jaeger` or `tempo`).
     4. Required for ambient mode with Istio 1.30 or later: waypoint proxies report every span under the waypoint's own service name (for example `waypoint.<namespace>`), so Kiali must look up traces by the waypoint name. The setting has no effect on sidecar-mode workloads, so it is safe to keep enabled in either mode.
     5. Specifies the internal URL for the Jaeger or Tempo API.
     6. The `external_url` that the Kiali UI uses when displaying Tracing UI links to the user. This URL must be accessible to clients external to the cluster (e.g. a browser) in order to generate valid links.
   </Callouts>
...
3. Run the following command to apply the configuration:
   ...
   ```bash {name=config-kiali:apply-tracing-patch}
   kubectl -n istio-system patch kiali kiali --type merge -p "$(cat kiali_cr.yaml)"
   ```

   **Example output:**

   ```text {name=config-kiali:apply-tracing-patch-output}
