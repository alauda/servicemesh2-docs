...
1. Update the `Kiali` resource spec configuration for tracing:
   ...
   **Example `Kiali` resource `spec` configuration for tracing**
   ...
   <Callouts>
     ...
     5. The `external_url` that the Kiali UI uses when displaying Tracing UI links to the user. This URL must be accessible to clients external to the cluster (e.g. a browser) in order to generate valid links.
   </Callouts>
