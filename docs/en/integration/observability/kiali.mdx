...
1. Retrieve platform and monitoring configuration from the cluster.

   ```bash
   # CA certificate for ACP - base64-encoded
   export PLATFORM_CA=$(kubectl -ncpaas-system get secret dex.tls -o jsonpath='{.data.ca\.crt}')
...
2. Create a `Secret` named `kiali` in the `istio-system` namespace for OpenID authentication:

   ```bash
   kubectl create secret generic kiali --from-literal="oidc-secret=$OIDC_CLIENT_SECRET" -nistio-system
   ```

3. Synchronize basic auth secret for accessing monitoring database:
   ...
   <Callouts>
     1. Create a `Secret` named `kiali-monitoring-basic-auth` in the `istio-system` namespace for basic authentication to monitoring database (Prometheus or VictoriaMetrics).
   </Callouts>
...
4. Create a file named kiali.yaml with the following content. Replace placeholder values as needed, or render the variables with `envsubst` before applying:
   ...
   spec:
     auth:
       openid:
         api_proxy: https://erebus.cpaas-system  # [!code callout]
   ...
   <Callouts>
     1. `api_proxy` points to **erebus** to map ACP user tokens to Kubernetes tokens.
     2. `api_proxy_ca_data` is the base64‑encoded CA certificate used by **erebus**.
     3. `insecure_skip_verify_tls` can be set to `false` when not using a self-signed CA certificate.
     4. `issuer_uri` is the OIDC issuer URL for **dex**.
     5. `client_id` is the OIDC client ID for **dex**.
     6. `replicas` specifies the number of replicas for the Kiali deployment, should be at least `2` in production environments.
     7. `username` references the monitoring basic‑auth username stored in the `kiali-monitoring-basic-auth` Secret.
     8. `password` references the monitoring basic‑auth password stored in the `kiali-monitoring-basic-auth` Secret.
     9. `url` is the monitoring endpoint for Prometheus or VictoriaMetrics.
   </Callouts>

5. Apply the configuration, render the manifest with `envsubst`:

   ```bash
   envsubst < kiali.yaml | kubectl apply -f -
   ```

6. When the `Kiali` resource is ready, access the Kiali dashboard at `<platform-url>/kiali`.
...
1. Update the `Kiali` resource spec configuration for tracing:
   ...
   **Example `Kiali` resource `spec` configuration for tracing**
   ...
   spec:
     external_services:
       tracing:
         ...
         use_grpc: true
         internal_url: "http://<jaeger-service-name>.<jaeger-service-namespace>:16685/jaeger"  # [!code callout]
   ...
   <Callouts>
     ...
     2. Specifies the tracing provider (`jaeger` or `tempo`).
     3. Specifies the internal URL for the Jaeger API.
   </Callouts>
...
3. Run the following command to apply the configuration:
   ...
   ```bash
   kubectl -ncpaas-system patch kiali kiali --type merge -p "$(cat kiali_cr.yaml)"
   ```
