...
<Steps>
  ...
  **Example output:**
  ...
  <Callouts>
    ...
    9. `replicas` specifies the number of replicas for the Kiali deployment, should be at least `2` in production environments. In that case, also uncomment `pod_disruption_budget` (supported since Kiali v2.27) so that voluntary node drains keep at least one Kiali pod running.
