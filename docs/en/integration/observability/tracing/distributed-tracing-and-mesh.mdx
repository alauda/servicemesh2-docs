...
The `Alauda build of Jaeger` (Jaeger 1.60.0) and `Alauda Build of OpenTelemetry` integration described in this document is deprecated and will be removed in a future release. Use the new <ExternalSiteLink name="distributed-tracing" href="/" children="Alauda Distributed Tracing" /> integration documented in [Distributed Tracing and Service Mesh (v2)](./distributed-tracing-and-mesh-v2.mdx) for all new deployments.

Choose the migration path that matches your operational needs:

- **You do not need to query historical trace data.** Uninstall the legacy `Alauda Build of OpenTelemetry` and `Alauda build of Jaeger` components by following [Uninstalling distributed tracing](#uninstalling-distributed-tracing) below, then deploy the new stack as described in [Distributed Tracing and Service Mesh (v2)](./distributed-tracing-and-mesh-v2.mdx).
...
:::warning
After uninstalling, historical traces stored in Elasticsearch are no longer reachable from the Jaeger UI. If you need to keep them, follow <ExternalSiteLink name="distributed-tracing" href="/migrating/migrating-from-acp-tracing.html" children="Migrating from Alauda Container Platform Tracing" /> instead.
:::
