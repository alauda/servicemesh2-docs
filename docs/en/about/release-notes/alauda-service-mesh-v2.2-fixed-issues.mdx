...
**Fixed in:** Istio v1.30.4. The agent now verifies the host rules periodically and reinstalls them when they no longer match. Set the interval in the `IstioCNI` resource, where `"0"` disables the check:
