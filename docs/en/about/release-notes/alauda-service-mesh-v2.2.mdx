...
- **Gateway API**: Istio 1.30 reads `TLSRoute` from `gateway.networking.k8s.io/v1`. Upgrade the Gateway API CRDs to bundle version `v1.5.x` before you update the control plane, otherwise TLS passthrough listeners silently report `attachedRoutes: 0`. See [Gateway API CRD requirements](../../updating/update-mesh/index.mdx#gateway-api-crd-requirements).
