...
- For gateways that use the `istio` gateway class:
    ...
    ```bash {name=psa:patch-gatewayclass-istio}
...
- For waypoint proxies, which use the `istio-waypoint` gateway class:
    ...
    ```bash {name=psa:patch-gatewayclass-waypoint}
...
```bash {name=psa:verify-gatewayclass-configmaps}
kubectl get configmap -n istio-system -l gateway.istio.io/defaults-for-class
```

**Example output**

```text {name=psa:verify-gatewayclass-configmaps-output}
NAME                                        DATA   AGE
