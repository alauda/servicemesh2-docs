...
1. Deploy the `tcp-echo` application with dual-stack configuration:
   ...
   kubectl apply -n dual-stack -f https://raw.githubusercontent.com/alauda-mesh/istio/istio-1.30/samples/tcp-echo/tcp-echo-dual-stack.yaml
   ```

2. Deploy the `tcp-echo` application with IPv4-only configuration:

   ```bash {name=dual-stack:deploy-tcp-echo-ipv4}
   kubectl apply -n ipv4 -f https://raw.githubusercontent.com/alauda-mesh/istio/istio-1.30/samples/tcp-echo/tcp-echo-ipv4.yaml
   ```

3. Deploy the `tcp-echo` application with IPv6-only configuration:

   ```bash {name=dual-stack:deploy-tcp-echo-ipv6}
   kubectl apply -n ipv6 -f https://raw.githubusercontent.com/alauda-mesh/istio/istio-1.30/samples/tcp-echo/tcp-echo-ipv6.yaml
   ```

4. Deploy the `sleep` application, which will act as a client for sending test requests:

   ```bash {name=dual-stack:deploy-sleep}
   kubectl apply -n sleep -f https://raw.githubusercontent.com/alauda-mesh/istio/istio-1.30/samples/sleep/sleep.yaml
   ```
