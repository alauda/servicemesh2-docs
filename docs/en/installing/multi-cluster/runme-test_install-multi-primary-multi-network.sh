...
# 合并 configuration-overview 与 install-multi-primary-multi-network 的全流程
...
# 通过 EAST_CLUSTER_NAME / WEST_CLUSTER_NAME 设置 CTX_CLUSTER1/2 与 ISTIO_VERSION
_setup_env() {
...
# 等待 LoadBalancer Service 拿到外部 IP
_wait_for_lb_ip() {
    local context="$1"
    local namespace="$2"
    local service="$3"
...
test_install_multi_primary_multi_network() {
    ...
    if [ -z "$WORK_DIR" ] || [ ! -d "$WORK_DIR" ]; then
        WORK_DIR=$(mktemp -d -t mp-mn-certs-XXXXXX)
    ...
    # ============================================================
    # Phase 3: 安装 Istio 多主多网络拓扑 (来自 install-mp-mn 文档)
    # ============================================================
    log_info "=== Phase 3: 安装 Istio 多主多网络拓扑 ==="
    ...
    log_info "步骤 3.12: 在 West 上安装 cluster1 remote secret"
    ...
    log_success "Phase 3 完成: 多主多网络网格已安装"
