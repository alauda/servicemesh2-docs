...
<Steps>
  ...
  ```bash {name=multi-primary-multi-network:set-istio-version}
  ...
  ```bash {name=multi-primary-multi-network:install-istio-cni-east}
  ...
  1. Create an `Istio` resource on the **East** cluster by running the following command:
     ...
     ```bash {name=multi-primary-multi-network:install-istio-east}
     cat <<EOF | kubectl --context "${CTX_CLUSTER1}" apply -f -
     apiVersion: sailoperator.io/v1
     kind: Istio
     metadata:
  ...
  2. Wait for the control plane to return the `Ready` status condition by running the following command:
     ...
     ```bash {name=multi-primary-multi-network:wait-istio-east}
  ...
  3. Create an East-West gateway on the **East** cluster by running the following command:
     ...
     ```bash {name=multi-primary-multi-network:create-eastwest-gw-east}
  ...
  4. Expose the services through the gateway by running the following command:
     ...
     ```bash {name=multi-primary-multi-network:expose-services-east}
  ...
  ```bash {name=multi-primary-multi-network:install-istio-cni-west}
  ...
  1. Create an `Istio` resource on the **West** cluster by running the following command:
     ...
     ```bash {name=multi-primary-multi-network:install-istio-west}
     cat <<EOF | kubectl --context "${CTX_CLUSTER2}" apply -f -
     apiVersion: sailoperator.io/v1
     kind: Istio
     metadata:
  ...
  2. Wait for the control plane to return the `Ready` status condition by running the following command:
     ...
     ```bash {name=multi-primary-multi-network:wait-istio-west}
  ...
  3. Create an East-West gateway on the **West** cluster by running the following command:
     ...
     ```bash {name=multi-primary-multi-network:create-eastwest-gw-west}
  ...
  4. Expose the services through the gateway by running the following command:
     ...
     ```bash {name=multi-primary-multi-network:expose-services-west}
  ...
  ```bash {name=multi-primary-multi-network:install-remote-secret-cluster2-on-east}
  ...
  ```bash {name=multi-primary-multi-network:install-remote-secret-cluster1-on-west}
  istioctl create-remote-secret \
...
1.  Create a dedicated namespace for the applications on the `East` cluster.
    ...
    ```shell {name=multi-primary-multi-network:create-sample-ns-east}
...
2.  Enable automatic Istio sidecar injection for the `sample` namespace by applying the required label.
    ...
    ```shell {name=multi-primary-multi-network:label-sample-ns-east}
...
3.  Deploy the `helloworld` application components.
    ...
    ```shell {name=multi-primary-multi-network:deploy-helloworld-svc-east}
    ...
    ```shell {name=multi-primary-multi-network:deploy-helloworld-v1-east}
    kubectl --context="${CTX_CLUSTER1}" apply \
...
4.  Deploy the `sleep` application, which will act as a client for sending test requests.
    ...
    ```shell {name=multi-primary-multi-network:deploy-sleep-east}
    kubectl --context="${CTX_CLUSTER1}" apply \
      -n sample \
...
5.  Pause the process until the `helloworld-v1` deployment is fully available and ready.
    ...
    ```shell {name=multi-primary-multi-network:wait-helloworld-v1-east}
...
6.  Likewise, wait for the `sleep` deployment to report a `Ready` status.
    ...
    ```shell {name=multi-primary-multi-network:wait-sleep-east}
...
1.  Create the `sample` namespace on the `West` cluster.
    ...
    ```shell {name=multi-primary-multi-network:create-sample-ns-west}
    kubectl --context="${CTX_CLUSTER2}" create namespace sample
    ```

2.  Enable Istio sidecar injection for this namespace as well.

    ```shell {name=multi-primary-multi-network:label-sample-ns-west}
...
3.  Deploy the `helloworld` application components.
    ...
    ```shell {name=multi-primary-multi-network:deploy-helloworld-svc-west}
    ...
    ```shell {name=multi-primary-multi-network:deploy-helloworld-v2-west}
    kubectl --context="${CTX_CLUSTER2}" apply \
...
4.  Deploy the client `sleep` application on the `West` cluster.
    ...
    ```shell {name=multi-primary-multi-network:deploy-sleep-west}
    kubectl --context="${CTX_CLUSTER2}" apply \
      -n sample \
      -f https://raw.githubusercontent.com/istio/istio/refs/heads/release-1.30/samples/sleep/sleep.yaml
    ```
...
5.  Wait for the `helloworld-v2` deployment to become fully available.
    ...
    ```shell {name=multi-primary-multi-network:wait-helloworld-v2-west}
...
6.  Finally, ensure the `sleep` deployment on the `West` cluster is ready.
    ...
    ```shell {name=multi-primary-multi-network:wait-sleep-west}
...
1.  From a pod within the `East` cluster, send a series of 10 requests to the `helloworld` service.
    ...
    ```shell {name=multi-primary-multi-network:test-traffic-east}
    for i in {0..9}; do \
    ...
    ```shell {name=multi-primary-multi-network:test-traffic-east-output}
...
2.  Perform the same test from the `West` cluster.
    ...
    ```shell {name=multi-primary-multi-network:test-traffic-west}
    for i in {0..9}; do \
...
1. Execute a single command to remove all Istio components and the sample applications from the `East` cluster.
   ...
   ```shell {name=multi-primary-multi-network:cleanup-east}
...
2. Run the corresponding command to perform the same cleanup operation on the `West` cluster.
   ...
   ```shell {name=multi-primary-multi-network:cleanup-west}
