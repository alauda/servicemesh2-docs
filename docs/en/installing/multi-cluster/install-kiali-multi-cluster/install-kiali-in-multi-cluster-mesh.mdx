...
<Steps>
  ...
  On success the script reports the server address it read from the context. The `kubectl` status word for the Secret can vary by apply mode and kubectl version (`created`, `configured`, or `serverside-applied`); the test verifies the Secret resource and its kubeconfig instead of relying on that status word.
