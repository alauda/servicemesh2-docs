...
- You have configured metrics collection by following [Configuring Monitoring with Service Mesh](../../../integration/observability/metrics-and-mesh.mdx). Kiali distinguishes clusters by the `source_cluster` and `destination_cluster` labels that Istio adds to these metrics, and it filters them by the `mesh_id` label, so where each resource goes matters:
  ...
  - Create the `ServiceMonitor` and `PodMonitor` objects on **every** cluster in the mesh, because monitoring scrapes each cluster separately. Leave the `mesh_id` relabeling of the `PodMonitor` object commented out, because the `Telemetry` resource already adds that label to the metrics of a sidecar.
  - Create the `Telemetry` resource, including its `mesh_id` tag override, on **every** cluster that runs a control plane. A sidecar receives its metrics configuration from the control plane that configures it, so metrics from a cluster whose control plane has no `Telemetry` resource carry no `mesh_id` label at all, and the query scope of Kiali then hides them. In a primary-remote mesh, the primary control plane also configures the sidecars of the remote clusters, and applying the resource on a remote cluster is rejected by the `validation.istio.io` webhook, since the `istiod` service there has no endpoints.
