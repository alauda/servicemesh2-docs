...
- You have access to two clusters with external load balancer support.
  :::warning
   **In primary-remote mode, if the external load balancer address is an IP and it is IPv6, primary-remote deployment is not supported in versions prior to Alauda Service Mesh v2.1.2**.
  :::
...
<Steps>
  ...
  ```bash {name=primary-remote-multi-network:set-istio-version}
  ...
  ```bash {name=primary-remote-multi-network:install-istio-cni-east}
  ...
  1. Create an `Istio` resource on the **East** cluster by running the following command:
     ...
     ```yaml title="istio-external.yaml" {name=primary-remote-multi-network:istio-external-yaml}
     apiVersion: sailoperator.io/v1
     kind: Istio
     metadata:
       name: default
     ...
     <Callouts>
       ...
       2. `spec.values.global.proxy.seccompProfile.type` set to `RuntimeDefault` lets the injected sidecars satisfy the `Restricted` Pod Security Admission profile. Because this control plane also injects the workloads of the remote **West** cluster, the setting covers both clusters. For more information, see [Namespace Requirements](../installing-service-mesh/application-deployment/namespace-requirements.mdx#pod-security-admission-modes).
     </Callouts>

     Using `kubectl` to apply the `Istio` resource:

     ```bash {name=primary-remote-multi-network:apply-istio-east}
  ...
  2. Wait for the control plane to return the "Ready" status condition by running the following command:
     ...
     ```bash {name=primary-remote-multi-network:wait-istio-east}
  ...
  3. Create an East-West gateway on the **East** cluster by running the following command:
     ...
     :::warning
     For nodes running Linux kernel versions earlier than 4.11 (e.g., CentOS 7), [additional configuration](../../gateways/gateway-installation/linux-kernel-compatibility-notice.mdx#linux-kernel-compatibility-notice) is required prior to gateway installation.
     :::

     ```bash {name=primary-remote-multi-network:create-eastwest-gw-east}
  ...
  4. Expose the control plane through the gateway so that services in the **West** cluster can access the control plane by running the following command:
     ...
     ```bash {name=primary-remote-multi-network:expose-istiod-east}
  ...
  5. Expose the application services through the gateway by running the following command:
     ...
     ```bash {name=primary-remote-multi-network:expose-services-east}
  ...
  ```bash {name=primary-remote-multi-network:install-istio-cni-west}
  ...
  1. Save the IP address of the East-West gateway running in the **East** cluster by running the following command:
     ...
     ```bash {name=primary-remote-multi-network:get-discovery-address}
  ...
  2. Create an `Istio` resource on the West cluster by running the following command:
     ...
     ```bash {name=primary-remote-multi-network:install-istio-west}
     cat <<EOF | kubectl --context "${CTX_CLUSTER2}" apply -f -
     apiVersion: sailoperator.io/v1
     kind: Istio
     metadata:
  ...
  3. Annotate the `istio-system` namespace in the **West** cluster so that it is managed by the control plane in the **East** cluster by running the following command:
     ...
     ```bash {name=primary-remote-multi-network:annotate-istio-system-west}
  ...
  4. Install a remote secret on the **East** cluster that provides access to the API server on the **West** cluster by running the following command:
     ...
     ```bash {name=primary-remote-multi-network:install-remote-secret-cluster2-on-east}
  ...
  5. Wait for the `Istio` resource to return the "Ready" status condition by running the following command:
     ...
     ```bash {name=primary-remote-multi-network:wait-istio-west}
  ...
  6. Create an East-West gateway on the **West** cluster by running the following command:
     ...
     ```bash {name=primary-remote-multi-network:create-eastwest-gw-west}
...
1.  Create a dedicated namespace for the applications on the `East` cluster.
    ...
    ```shell {name=primary-remote-multi-network:create-sample-ns-east}
...
2.  Enable automatic Istio sidecar injection for the `sample` namespace by applying the required label.
    ...
    ```shell {name=primary-remote-multi-network:label-sample-ns-east}
...
3.  Deploy the `helloworld` application components.
    ...
    ```shell {name=primary-remote-multi-network:deploy-helloworld-svc-east}
    kubectl --context="${CTX_CLUSTER1}" apply \
      -l service=helloworld -n sample \
      -f https://raw.githubusercontent.com/istio/istio/refs/heads/release-1.30/samples/helloworld/helloworld.yaml
    ```

    b.  Then, deploy the `v1` instance of the `helloworld` application.

    ```shell {name=primary-remote-multi-network:deploy-helloworld-v1-east}
    kubectl --context="${CTX_CLUSTER1}" apply \
...
4.  Deploy the `sleep` application, which will act as a client for sending test requests.
    ...
    ```shell {name=primary-remote-multi-network:deploy-sleep-east}
    kubectl --context="${CTX_CLUSTER1}" apply \
      -n sample \
      -f https://raw.githubusercontent.com/istio/istio/refs/heads/release-1.30/samples/sleep/sleep.yaml
    ```

5.  Pause the process until the `helloworld-v1` deployment is fully available and ready.

    ```shell {name=primary-remote-multi-network:wait-helloworld-v1-east}
...
6.  Likewise, wait for the `sleep` deployment to report a `Ready` status.
    ...
    ```shell {name=primary-remote-multi-network:wait-sleep-east}
...
1.  Create the `sample` namespace on the `West` cluster.
    ...
    ```shell {name=primary-remote-multi-network:create-sample-ns-west}
    kubectl --context="${CTX_CLUSTER2}" create namespace sample
    ```

2.  Enable Istio sidecar injection for this namespace as well.

    ```shell {name=primary-remote-multi-network:label-sample-ns-west}
...
3.  Deploy the `helloworld` application components.
    ...
    ```shell {name=primary-remote-multi-network:deploy-helloworld-svc-west}
    ...
    ```shell {name=primary-remote-multi-network:deploy-helloworld-v2-west}
    kubectl --context="${CTX_CLUSTER2}" apply \
...
4.  Deploy the client `sleep` application on the `West` cluster.
    ...
    ```shell {name=primary-remote-multi-network:deploy-sleep-west}
    kubectl --context="${CTX_CLUSTER2}" apply \
      -n sample \
      -f https://raw.githubusercontent.com/istio/istio/refs/heads/release-1.30/samples/sleep/sleep.yaml
    ```
...
5.  Wait for the `helloworld-v2` deployment to become fully available.
    ...
    ```shell {name=primary-remote-multi-network:wait-helloworld-v2-west}
...
6.  Finally, ensure the `sleep` deployment on the `West` cluster is ready.
    ...
    ```shell {name=primary-remote-multi-network:wait-sleep-west}
...
1.  From a pod within the `East` cluster, send a series of 10 requests to the `helloworld` service.
    ...
    ```shell {name=primary-remote-multi-network:test-traffic-east}
    for i in {0..9}; do \
    ...
    ```shell {name=primary-remote-multi-network:test-traffic-east-output}
...
2.  Perform the same test from the `West` cluster.
    ...
    ```shell {name=primary-remote-multi-network:test-traffic-west}
    for i in {0..9}; do \
...
1. Execute a single command to remove all Istio components and the sample applications from the `East` cluster.
   ...
   ```shell {name=primary-remote-multi-network:cleanup-east}
...
2. Run the corresponding command to perform the same cleanup operation on the `West` cluster.
   ...
   ```shell {name=primary-remote-multi-network:cleanup-west}
