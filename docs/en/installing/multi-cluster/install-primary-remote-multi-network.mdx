...
1.  Create a dedicated namespace for the applications on the `East` cluster.
    ...
    kubectl --context="${CTX_CLUSTER1}" label namespace sample pod-security.kubernetes.io/enforce=restricted
    ```
...
1.  Create the `sample` namespace on the `West` cluster.
    ...
    kubectl --context="${CTX_CLUSTER2}" label namespace sample pod-security.kubernetes.io/enforce=restricted
    ```
