...
test_deploying_bookinfo() {
    ...
    if [ "${ENABLE_METALLB:-false}" = "true" ]; then
        ...
        # 步骤 22: 获取 GATEWAY_URL（按 INGRESS_HOST 是否为 IPv6 选择对应代码块）
        log_info "步骤 22: 获取 GATEWAY_URL"
        ...
        # 步骤 23: 打印 productpage 完整 URL（覆盖文档 echo 代码块）
        log_info "步骤 23: 打印 productpage 完整 URL"
        runme run bookinfo:gw-api-echo-url || {
            log_error "打印完整 URL 失败"
            return 1
        }

        # 步骤 24: 验证——取文档验证命令（bookinfo:gw-api-verify），经 ratings 服务在集群内执行，
        #          断言 bookinfo:gw-api-verify-output（命令内 ${GATEWAY_URL} 由本 shell 展开）
        log_info "步骤 24: 经 ratings 服务验证 Gateway API 访问"
