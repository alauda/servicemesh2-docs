...
1.  Use the following command to Create a new namespace named `bookinfo`:
    ...
    ```shell {name=bookinfo:create-namespace}
...
2.  Enable sidecar injection for the `bookinfo` namespace. If your setup uses the `InPlace` upgrade strategy, run this command:
    ...
    ```shell {name=bookinfo:enable-sidecar-injection-inplace}
    ...
    1.  To discover your `<revision-name>`, run the following:
        ...
        ```shell {name=bookinfo:get-istio-revisions}
        kubectl get istiorevisions.sailoperator.io
        ```

        **Sample output:**

        ```shell {name=bookinfo:get-istio-revisions-output}
    ...
    2.  Label the namespace using the revision name to enable sidecar injection:
        ...
        ```shell {name=bookinfo:enable-sidecar-injection-revision}
...
3.  *Optional* : If you have already configured `discoverySelectors`, use the following command to apply the Istio `discovery selector` to the `bookinfo` namespace:
    ...
    **Example `discovery selector` label**: `istio-discovery=enabled`
    ...
    ```shell {name=bookinfo:apply-discovery-selector}
...
4.  Deploy the `bookinfo` application by applying its YAML file with the following command:
    ...
    ```shell {name=bookinfo:deploy-application}
...
1.  To confirm the `bookinfo` service is available, run this command:
    ...
    ```shell {name=bookinfo:verify-services}
    kubectl get services -n bookinfo
    ```

    **Example output**

    ```shell {name=bookinfo:verify-services-output}
...
2.  To check that the `bookinfo` pods are available, execute the following command:
    ...
    ```shell {name=bookinfo:verify-pods}
    kubectl get pods -n bookinfo
    ```

    **Example output**

    ```shell {name=bookinfo:verify-pods-output}
...
3.  Confirm the `bookinfo` application is running by sending a request to its `product page`. Execute this command:
    ...
    ```shell {name=bookinfo:verify-application}
    ...
    **Example output**
    ...
    ```html {name=bookinfo:verify-application-output}
...
There are two methods for deploying gateways: [using the Gateway API](../../../gateways/directing-traffic-into-the-mesh/exposing-a-service-via-k8s-gateway-api-in-sidecar-mode.mdx) or [the gateway injection technique](../../../gateways/gateway-installation/installing-a-gateway-via-injection.mdx).
...
1.  Execute the following command to create the `istio-ingressgateway` deployment and service:
    ...
    ```shell {name=bookinfo:gw-inject-create-gateway}
...
2.  Configure the `bookinfo` application to use the newly created gateway by running this command:
    ...
    ```shell {name=bookinfo:gw-inject-configure}
...
3.  *Optional* : Modify the YAML file for automatic pod scaling based on ingress traffic.
    ...
    **Example configuration**
    ...
    ```yaml {name=bookinfo:gw-inject-hpa-yaml}
...
4.  *Optional* : Define the minimum number of pods that should be running on the node.
    ...
    **Example configuration**
    ...
    ```yaml {name=bookinfo:gw-inject-pdb-yaml}
...
2.  Create and configure a gateway with the `Gateway` and `HTTPRoute` resources by executing the command below:
    ...
    ```shell {name=bookinfo:gw-api-create-gateway}
...
5.  Retrieve the port with this command:
    ...
    ```shell {name=bookinfo:gw-api-get-port}
...
6.  Retrieve the gateway URL with this command:
    ...
    ```shell {name=bookinfo:gw-api-get-url-ipv6}
    export GATEWAY_URL=[$INGRESS_HOST]:$INGRESS_PORT
    ```
    :::

    ```shell {name=bookinfo:gw-api-get-url}
    export GATEWAY_URL=$INGRESS_HOST:$INGRESS_PORT
    ```

7.  Get the full URL for the product page by running the following command:

    ```shell {name=bookinfo:gw-api-echo-url}
...
#### Verification
  ...
  - Confirm that the **productpage** is accessible with the `curl` command:
    ...
    ```shell {name=bookinfo:gw-api-verify}
    curl -sS -g "http://${GATEWAY_URL}/productpage" | grep -o "<title>.*</title>"
    ```

    **Example output**

    ```html {name=bookinfo:gw-api-verify-output}
    <title>Simple Bookstore App</title>
    ```
...
## Removing the Bookinfo Application \{#removing-the-bookinfo-application}
