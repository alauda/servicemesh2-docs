...
10. Add the following YAML snippet to the **YAML** code editor:
    ...
    <Callouts>
      1. `spec.values.global.proxy.seccompProfile.type` set to `RuntimeDefault` adds the `RuntimeDefault` seccomp profile to the `istio-validation` and `istio-proxy` containers that the control plane injects into application pods. Injected sidecars then satisfy the `Restricted` Pod Security Admission (PSA) profile. Omit this setting only if no application namespace enforces the `Restricted` profile. For more information, see [Namespace Requirements](./application-deployment/namespace-requirements.mdx#pod-security-admission-modes).
    </Callouts>
11. Click **Create**.
