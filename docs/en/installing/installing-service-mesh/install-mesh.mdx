...
:::warning
Do not install Alauda Service Mesh v2 into a cluster that already runs Alauda Service Mesh, as this may result in functional conflicts.

The two generations run side by side only while a mesh is being migrated from Alauda Service Mesh to Alauda Service Mesh v2, and only by following [Migrating](../../migrating/index.mdx), which installs the control plane with the settings of the existing mesh and removes the old generation at the end. Do not install Alauda Service Mesh v2 next to an existing mesh in any other way.
:::

:::note
Confirm that the cluster version matches the Istio version you plan to deploy: Istio 1.30 supports Kubernetes 1.32 to 1.36, and Istio 1.28 supports Kubernetes 1.30 to 1.34.
:::
...
CSV="$(kubectl -n sail-operator get subscription servicemesh-operator2 -o jsonpath='{.status.currentCSV}')"
kubectl -n sail-operator wait --for=create "csv/${CSV}" --timeout=2m
kubectl -n sail-operator wait --for=jsonpath='{.status.phase}'=Succeeded "csv/${CSV}" --timeout=3m
```
...
- **Sail Operator CRDs**:
  ...
  Refer to the [Sail Operator API reference](https://github.com/alauda-mesh/sail-operator/blob/release-2.2/docs/api-reference/sailoperator.io.md) for details.
...
10. Add the following YAML snippet to the **YAML** code editor:
    ...
    <Callouts>
      1. `spec.values.global.proxy.seccompProfile.type` set to `RuntimeDefault` adds the `RuntimeDefault` seccomp profile to the `istio-validation` and `istio-proxy` containers that the control plane injects into application pods. Injected sidecars then satisfy the `Restricted` Pod Security Admission (PSA) profile. Omit this setting only if no application namespace enforces the `Restricted` profile. For more information, see [Pod Security Admission](../pod-security-admission.mdx#sidecars).
    </Callouts>
11. Click **Create**.
