...
9. Update the `Istio` custom resource (CR) so it resembles the following example:
    ...
    ```yaml {name=istio-ha-replica-count:console-yaml}
...
1. Retrieve the name of the `Istio` resource:
    ...
    ```bash {name=istio-ha-replica-count:get-istio}
    kubectl get istio -n istio-system
    ```

    **Example output**

    ```bash {name=istio-ha-replica-count:get-istio-output}
...
2. Patch the `Istio` custom resource (CR) to disable autoscaling and set a fixed replica count:
    ...
    ```bash {name=istio-ha-replica-count:patch-istio}
...
```bash {name=istio-ha-replica-count:verify-istiod-pods}
kubectl get pods -n istio-system -l app=istiod
```

**Example output**

```bash {name=istio-ha-replica-count:verify-istiod-pods-output}
