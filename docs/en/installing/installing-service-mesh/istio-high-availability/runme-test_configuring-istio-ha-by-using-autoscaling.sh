...
test_configuring_istio_ha_by_using_autoscaling() {
    ...
    local console_yaml
    console_yaml=$(runme print istio-ha-autoscaling:console-yaml 2>&1) || {
...
EOF
    ...
    istio_output=$(runme run istio-ha-autoscaling:get-istio 2>&1) || {
        log_error "获取 Istio 资源失败"
        log_error "输出: $istio_output"
        return 1
    }
    expected_istio_output=$(runme print istio-ha-autoscaling:get-istio-output 2>&1) || {
        log_error "获取预期输出失败"
...
EOF
    ...
    # 3. 执行 patch 命令
    log_info "步骤 3: 为 Istio 控制面启用自动伸缩"
    local patch_output
    patch_output=$(runme run istio-ha-autoscaling:patch-istio 2>&1) || {
    ...
    expected_verify_output=$(runme print istio-ha-autoscaling:verify-istiod-pods-output 2>&1) || {
    ...
    verify_output=$(runme run istio-ha-autoscaling:verify-istiod-pods 2>&1) || {
...
EOF
    ...
    log_success "=========================================="
    log_success "使用自动伸缩配置 Istio 高可用测试完成，所有验证通过！"
    log_success "=========================================="
    return 0
}
