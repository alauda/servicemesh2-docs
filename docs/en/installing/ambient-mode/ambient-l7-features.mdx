...
1. Create the traffic routing configuration. Save the following as `traffic-route.yaml`:
    ...
    ```yaml {name=ambient-l7-features:traffic-route-yaml}
...
2. Apply the traffic routing configuration:
    ...
    ```bash {name=ambient-l7-features:apply-traffic-route}
...
```bash {name=ambient-l7-features:verify-traffic-split}
...
```bash {name=ambient-l7-features:cleanup-traffic-route}
...
1. Create the authorization policy. Save the following as `authorization-policy.yaml`:
    ...
    ```yaml {name=ambient-l7-features:authz-policy-yaml}
...
2. Apply the authorization policy:
    ...
    ```bash {name=ambient-l7-features:apply-authz-policy}
...
1. Create a namespace for the curl client:
    ...
    ```bash {name=ambient-l7-features:create-ns-curl}
    kubectl create namespace curl
    ```

2. Deploy the curl client:

    ```bash {name=ambient-l7-features:deploy-curl}
...
3. Add the `istio-discovery=enabled` label to the `curl` namespace:
    ...
    ```bash {name=ambient-l7-features:label-curl-discovery}
    kubectl label namespace curl istio-discovery=enabled
    ```

4. Enable ambient mode for the `curl` namespace:

    ```bash {name=ambient-l7-features:label-curl-ambient}
...
5. Verify that a GET request to the `productpage` service succeeds with an `HTTP 200` response:
    ...
    ```bash {name=ambient-l7-features:verify-get-allowed}
    ...
    **Expected output**
    ...
    ```text {name=ambient-l7-features:verify-get-allowed-output}
    HTTP 200
    ```

6. Verify that a POST request to the same service is denied with an `HTTP 403` response:

    ```bash {name=ambient-l7-features:verify-post-denied}
    kubectl -n curl exec deploy/curl -- sh -c '\
      curl -s -o /dev/null -w "HTTP %{http_code}\n" \
        -X POST \
        http://productpage.bookinfo.svc.cluster.local:9080/productpage'
    ...
    **Expected output**
    ...
    ```text {name=ambient-l7-features:verify-post-denied-output}
    HTTP 403
    ```
...
7. Verify that a GET request from another service (such as the `ratings` pod in the `bookinfo` namespace) is also denied with `RBAC: access denied`:
    ...
    ```bash {name=ambient-l7-features:verify-get-denied}
    ...
    **Expected output**
    ...
    ```text {name=ambient-l7-features:verify-get-denied-output}
    RBAC: access denied
    ```

**Clean up the verification resources**

```bash {name=ambient-l7-features:cleanup-authorization-policy}
...
kubectl delete -n curl -f https://raw.githubusercontent.com/istio/istio/refs/heads/release-1.30/samples/curl/curl.yaml
