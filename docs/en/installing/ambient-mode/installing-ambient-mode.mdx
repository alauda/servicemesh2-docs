...
2. Create the ZTunnel resource named `ztunnel.yaml`:
    ...
    <Callouts>
        1. The `requests` values shown here are the chart defaults. The chart does not define any `limits` by default, so declare them explicitly; `2000m` / `1024Mi` matches the default limits of the other data plane proxies (sidecar and waypoint). Ztunnel runs as a per-node proxy whose memory grows with the number of open connections on the node, so raise the memory limit accordingly in high-traffic clusters.
    </Callouts>
