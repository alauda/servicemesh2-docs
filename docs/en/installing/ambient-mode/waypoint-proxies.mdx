...
1. Add the `istio-waypoint` gateway class overlay to the `Istio` resource so that the waypoint pod is admitted into the `Restricted` `bookinfo` namespace:

    ```bash {name=ambient-waypoint:patch-gatewayclass}
