...
1. *Optional* : If the `bookinfo` namespace enforces the `Restricted` Pod Security Admission profile, add the `istio-waypoint` gateway class overlay to the `Istio` resource. Without it, the waypoint pod is rejected:
...
2. Create a waypoint proxy in the `bookinfo` namespace. Save the following as `waypoint.yaml`:
...
3. Apply the waypoint custom resource (CR):
