...
default-v1-30-3   istio-system             True    Healthy   False    v1.30.3   40s
```
...
For instance, using the example revision `default-v1-30-3` from above, the following labels would enable sidecar injection:

| **Resource** | **Enabled label**              | **Disabled label**              |
| ------------ | ------------------------------ | ------------------------------- |
| Namespace    | `istio.io/rev=default-v1-30-3` | `istio-injection=disabled`      |
| Pod          | `istio.io/rev=default-v1-30-3` | `sidecar.istio.io/inject=false` |
