...
1.  To find the name of your `Istio` resource, execute the following command:
    ...
    **Example output**
    ...
    default   istio-system             1           1       0        default-v1-28-1   Healthy   v1.28.1   37s
    ```

    In this case, the `Istio` resource is named `default`, but its underlying revision is `default-v1-28-1`.
...
4.  Confirm the successful creation of the `IstioRevisionTag` resource with the following command:
    ...
    **Example output**
    ...
    default   Healthy   True     default-v1-28-1   15s
    ```

    As shown in the example, the new tag now references your active revision, `default-v1-28-1`. Now you can use the `istio-injection=enabled` label as if your revision was called `default`.
