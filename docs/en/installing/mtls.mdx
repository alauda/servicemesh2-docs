...
- On the **Namespaces** page, the **mTLS** column reports the mode of each namespace, such as `Strict`, `Permissive`, or `Disabled`. The namespace detail page reports the same value in its **mTLS** field. Hover over the lock icon to see whether the mode is set on the namespace or inherited from the mesh-wide configuration.
- On the **Mesh** page, select the control plane node to read the mesh-wide mTLS status in its side panel.
