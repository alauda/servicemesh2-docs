...
- **不存在整集群断流窗口**。各组件滚动期间，数据面始终保留 L4 转发与 mTLS 能力；ZTunnel 与 CNI 按节点逐个滚动，任一时刻最多影响一个节点；waypoint 按 Deployment 滚动，新 Pod 就绪后旧 Pod 才退出。
- **最主要的业务流量影响环节是 ZTunnel 替换**（未部署 waypoint 时也是唯一环节）：ZTunnel 工作在 L4，已建立的 TCP 连接无法移交给新进程，超过排水期的长连接会被强制断开（reset）。
- **如部署了 waypoint，存在一个次要影响点**：控制面升级后 istiod 自动滚动 waypoint Deployment，影响范围仅限经由该 waypoint 的 L7 流量——新 Pod 就绪后旧 Pod 才优雅排水，短请求基本无感，但长连接仍会随旧 Pod 终止而断开重连。影响程度远小于 ZTunnel 的 L4 强制 reset（详见 3.2 分组件表）。
