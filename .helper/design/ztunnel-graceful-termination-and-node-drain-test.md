...
因此场景一用**有限寿命连接**（`iperf3 -t 60`，跑 60 秒后自然结束），场景二用**常驻连接**（`ncat` 心跳，永不主动关闭）。
...
- 测试镜像：[`nicolaka/netshoot`](https://hub.docker.com/r/nicolaka/netshoot)（k8s 网络排障标准工具箱，内置 `ncat`、`socat`、`iperf3`、`tcpdump` 等，一镜像即可同时充当 server / client / 抓包端）。
- 集群可拉取该镜像；若处于离线环境，请先把镜像同步到内部 registry 并替换下文 manifest 中的 `image`。

> 选择 `nicolaka/netshoot` 的理由：本测试的目的就是**观测** RST/FIN，`ncat`/`iperf3` 能精确控制连接寿命并给出清晰的关闭原因，`tcpdump` 可做包级佐证，全部在同一个广泛使用的镜像里，无需引入额外的应用或 broker。
    ...
    spec:
      ...
      containers:
        - name: net
          ...
          args:
            - |
              echo "[server] iperf3 -s :5201 + ncat echo :8080"
              iperf3 -s -p 5201 &
              exec ncat -lk -p 8080 --sh-exec "cat"   # 回显服务，-k 持续接受连接
---
apiVersion: v1
kind: Service
metadata:
        ...
        - name: net
          ...
          args:
            - |
              ...
              # 外层重连模拟"带重试能力的应用"；判定单条连接的死法只看每次 ncat 的退出码与 stderr。
              while true; do
                echo "===== [$(date '+%F %T')] OPEN long-lived TCP -> ${SERVER}:${PORT} ====="
                ( while true; do echo "ping $(date '+%T')"; sleep 2; done ) | ncat -v ${SERVER} ${PORT}
                echo "===== [$(date '+%F %T')] CLOSED (ncat exit=$?) — 见上方 stderr 判定 RST/FIN；3 秒后重连 ====="
                sleep 3
              done
```
...
echo "server 在 $NODE_SERVER ；client 在 $NODE_CLIENT"
```

> 后续步骤分布在多个终端。`NODE_SERVER` / `NODE_CLIENT` 是 shell 变量，**每开一个新终端都要重新执行上面的 export**（或直接把命令里的 `$NODE_xxx` 替换成实际节点名）。注意：场景二 drain 节点后 Pod 会重新调度，节点可能变化，每个场景开始前重新取一次值最稳妥。
...
3. **终端 2** —— iperf3 起跑约 5 秒后，替换 client 所在节点的 ztunnel（最快、版本无关的触发方式）：
   ...
   ```bash
   kubectl -n ztunnel delete pod --field-selector spec.nodeName=$NODE_CLIENT
   ```

4. 观测终端 1：约 **30 秒**后（排水期满），iperf3 在跑满 60 秒前被中止并报错退出（`Connection reset by peer` / `control socket has closed unexpectedly`）。同时 client 日志里常驻 `ncat` 也会出现一次 `exit=1` + `reset by peer`。
...
5. 观测终端 1：旧 ztunnel 在排水期（120s）内持续服务该连接，iperf3 在 **60 秒**时正常跑完、打印完整 summary、退出 `0`，**全程无 reset**（因 60s < 120s）。
   ...
   **结论**：把排水期调到覆盖连接寿命，有限寿命长连接在排水期内自然结束，不被 RST。
   ...
   > 教学点：此时 client 那条**常驻** `ncat` 连接仍会在替换后第 120 秒被 RST——因为它是无限寿命连接，排水期再大也救不了。这正是为什么需要场景二。
...
用常驻 `ncat` 连接（无限寿命，场景一证明它靠调大排水期救不了）。这次**排空 server 所在节点 `$NODE_SERVER`**：server 被优雅驱逐 → 连接由应用端关闭（FIN）→ client 在 `$NODE_CLIENT` 上始终在线，充当观察者见证"优雅关闭而非 RST"。ztunnel 排水期在本场景**不起作用**（换 ztunnel 前节点上已无工作负载）。
...
4. 排空 server 所在节点。`--ignore-daemonsets` 让 ztunnel / istio-cni 等 DaemonSet 保留在节点上（预期行为）：
   ...
   观测 client 日志：在 server 被驱逐的瞬间，常驻连接关闭，且 **`ncat exit=0`、无 `reset by peer`**——即对端（应用）优雅关闭（FIN）。随后 server Pod 因反亲和被重新调度到第三个节点，client 自动重连成功。
...
7. 对集群中其余每个节点重复步骤 4–6（逐节点排空 → 删旧 ztunnel Pod → uncordon），直到所有节点的 ztunnel 都更新到目标版本：
...
| 6 节点排空 | 不涉及 | 常驻 (ncat) | drain server 节点 | exit=0 / 无 reset（FIN） | 随 drain 即时、应用优雅 | ☐ |
...
- **必须用 `ncat`（nmap），不要用 Alpine busybox 的 `nc`**：后者缺少 `-v`/`-k` 等关键能力，关闭原因不易判读。`nicolaka/netshoot` 自带 `ncat`。
- **客户端自动重连会"掩盖"reset**：本测试客户端外层有重连循环（模拟带重试的应用），判定单条连接的死法**只看每次 `ncat` 的退出码与 stderr**，不要被"很快又连上了"误导。要模拟脆弱的单连接应用，只观察第一条连接的结局即可。
...
- [nicolaka/netshoot](https://hub.docker.com/r/nicolaka/netshoot) — 测试镜像（含 ncat / iperf3 / socat / tcpdump）
