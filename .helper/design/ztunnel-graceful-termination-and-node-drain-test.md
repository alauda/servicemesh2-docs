...
```bash
kubectl run nettool-check --rm -i --restart=Never --image=docker-mirrors.alauda.cn/nicolaka/netshoot -- \
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: net
          image: docker-mirrors.alauda.cn/nicolaka/netshoot:latest
          command: ["/bin/bash", "-lc"]
          args:
            - |
...
istioctl -n ztunnel ztunnel-config workloads | grep conntest
